...
pub async fn create_set(State(pool): State<Pool>, ValidatedJson(set): ValidatedJson<CreateSet>) -> impl IntoResponse {
...
pub async fn update_set(State(pool): State<Pool>, ValidatedJson(set): ValidatedJson<UpdateSet>) -> impl IntoResponse {
    set.update_one(&pool)
        .await
        .map(or_404::<_, Json<_>>)
        .log_error()
